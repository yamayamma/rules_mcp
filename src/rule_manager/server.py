...
from .models.base import (
    ...
    RuleEvaluationSummary, PriorityTieBreaking
)
...
class RuleManagerServer:
    ...
    def _register_tools(self):
        ...
        async def update_rule(request: UpdateRuleRequest) -> Dict[str, Any]:
            ...
            try:
                # Get existing rule
                existing_rule = await self.rule_store.get_rule(request.name, request.scope)
                ...
                for field, value in request.model_dump(exclude_unset=True).items():
                    if field != "name" and field != "scope":  # Don't allow changing name/scope
        ...
        @self.mcp.tool()
        async def get_rule(rule_name: str, scope: Optional[str] = None) -> Dict[str, Any]:
            """
