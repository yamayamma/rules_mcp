...
class TestDSLEvaluator:
    ...
    def test_in_operator(self):
        assert self.evaluator.evaluate('model_name in ["gpt-4", "claude"]', self.context) is True
        assert self.evaluator.evaluate('model_name in ["gpt-3.5-turbo"]', self.context) is False
        assert self.evaluator.evaluate('user_id in ["user123", "user456"]', self.context) is True

    def test_not_in_operator(self):
        assert self.evaluator.evaluate('model_name not in ["gpt-3.5-turbo"]', self.context) is True
        assert self.evaluator.evaluate('model_name not in ["gpt-4", "claude"]', self.context) is False

    def test_contains_operator(self):
        context = RuleContext(custom_attributes={"text": "hello world"})
        assert self.evaluator.evaluate('custom_attributes.text contains "world"', context) is True
        assert self.evaluator.evaluate('custom_attributes.text contains "xyz"', context) is False

    def test_string_methods(self):
        assert self.evaluator.evaluate('user_id startswith "user"', self.context) is True
        assert self.evaluator.evaluate('user_id endswith "123"', self.context) is True
        assert self.evaluator.evaluate('user_id startswith "admin"', self.context) is False
    ...
    def test_logical_not_operator(self):
        assert self.evaluator.evaluate('not user_id == "other_user"', self.context) is True
        assert self.evaluator.evaluate('not user_id == "user123"', self.context) is False

    def test_complex_logical_expression(self):
        expr = 'user_id == "user123" and (model_name == "gpt-4" or prompt_length > 2000)'
        assert self.evaluator.evaluate(expr, self.context) is True
    ...
    def test_custom_attributes_access(self):
        assert self.evaluator.evaluate('custom_attributes.environment == "production"', self.context) is True
        assert self.evaluator.evaluate('custom_attributes.user_role == "admin"', self.context) is True
        assert self.evaluator.evaluate('custom_attributes.user_clearance_level >= 3', self.context) is True

    def test_nested_attribute_access(self):
        # Test direct custom attribute access
        assert self.evaluator.evaluate('environment == "production"', self.context) is True
        assert self.evaluator.evaluate('user_role == "admin"', self.context) is True

    def test_list_handling(self):
        assert self.evaluator.evaluate('model_name in available_models', self.context) is True
        
        context_with_list = RuleContext(custom_attributes={"models": ["gpt-4", "claude"]})
        assert self.evaluator.evaluate('custom_attributes.models contains "gpt-4"', context_with_list) is True

    def test_boolean_values(self):
        context = RuleContext(custom_attributes={"enabled": True, "disabled": False})
        assert self.evaluator.evaluate('custom_attributes.enabled == true', context) is True
        assert self.evaluator.evaluate('custom_attributes.disabled == false', context) is True
