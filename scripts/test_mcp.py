...
async def test_mcp_tools():
    ...
    try:
        ...
        if "error" in get_result:
            print(f"   ⚠️ Rule not found (expected if creation failed): {get_result['error']}")
        else:
    ...
    try:
        delete_result = await server.mcp.tools["delete_rule"]("test_rule_dynamic", "global")
        
        if "error" in delete_result:
            print(f"   ⚠️ Rule deletion failed (may not exist): {delete_result['error']}")
        else:
            ...
            if "error" in result:
                ...
                for rule_result in result.get("results", [])[:3]:
                    if rule_result["matched"]:
                        print(f"   🎯 {rule_result['rule_name']}: {rule_result['action']}")
